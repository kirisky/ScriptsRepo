...
# auxdata_all: auxdata_common auxdata_meris auxdata_modisa auxdata_seawifs auxdata_viirs auxdata_msi




function Auxdata-Generic-Check
{
...
function Auxdata-Generic($fileName)
{
	# $WGET auxdata/generic/ $(URL)/generic/LUT.hdf
    wget ($URL + "/generic/" + $fileName) -OutFile ("auxdata/generic/" + $fileName)
}

function Auxdata-Common-Check
{
...
function Auxdata-Common($fileName)
{
    ...
    wget ($URL + "/common/" + $fileName) -OutFile ("auxdata/common/" + $fileName)
    
}
	

function Auxdata-MerisSmileV2-Check
{
    mkdir -p auxdata/meris/smile/v2
    Auxdata-MerisSmileV2("sun_spectral_flux_rr.txt")
    Auxdata-MerisSmileV2("central_wavelen_rr.txt")
    Auxdata-MerisSmileV2("sun_spectral_flux_fr.txt")
    Auxdata-MerisSmileV2("central_wavelen_fr.txt")
...
function Auxdata-MerisSmileV2($fileName)
{
    ...
    wget ($URL + "/meris/smile/v2/" + $fileName) -OutFile ("auxdata/meris/smile/v2/" + $fileName)
}
	

function Auxdata-Modisa-Check
{
...
function Auxdata-Modisa($fileName)
{
    # $(WGET) auxdata/modisa/ $(URL)/modisa/HMODISA_RSRs.txt
    wget ($URL + "/modisa/" + $fileName) -OutFile ("auxdata/modisa/" + $fileName)
}
	

function Auxdata-Seawifs-Check
{
...
function Auxdata-Seawifs($fileName)
{
    # $(WGET) auxdata/seawifs/ $(URL)/seawifs/SeaWiFS_RSRs.txt
    wget ($URL + "/seawifs/" + $fileName) -OutFile ("auxdata/seawifs/" + $fileName)
}
	

function Auxdata-Viirs-Check
{
...
function Auxdata-Viirs($fileName)
{
    # $(WGET) auxdata/viirs/ $(URL)/viirs/VIIRSN_IDPSv3_RSRs.txt
    wget ($URL + "/viirs/" + $fileName) -OutFile ("auxdata/viirs/" + $fileName)
}
	

function Auxdata-Msi-Check
{
    mkdir -p auxdata/msi
...
function Auxdata-Msi($fileName)
{
    ...
    wget ($URL + "/msi/" + $fileName) -OutFile ("auxdata/msi/" + $fileName)
}


echo "Getting generic data"
Auxdata-Generic-Check

echo "Getting common data"
Auxdata-Common-Check

echo "Getting meris/smile/v2/ data"
Auxdata-MerisSmileV2-Check

echo "Getting modisa data"
Auxdata-Modisa-Check

echo "Getting seawifs data"
Auxdata-Seawifs-Check

echo "Getting viirs data"
Auxdata-Viirs-Check

echo "Getting msi data"
Auxdata-Msi-Check